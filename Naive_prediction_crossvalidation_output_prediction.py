...
def main():
	parser = argparse.ArgumentParser(description='This code contains naive prediction approaches. ')
	# Required
	req_group = parser.add_argument_group(title='REQUIRED INPUT')
	req_group.add_argument('-path', help='directory where the expression/co-expression matrix are', required=True)
	req_group.add_argument('-expression', help='the expression/co-expression matrix', required=True)
	...
	Gene_prediction = {}  # Gene_prediction[gene1][Median_1] = pathway
	for cv_number in range(1,6):
		gene_list = open('Pathway_genes_for_testing.txt','r').readlines()
		D = {} #D[gene] = pathway
		P = {} #P[pathway] = [gene1,gene2,...]
		for inl in gene_list:
			gene = inl.split('\t')[1].strip()
	...
	out = open('./Result_on_test_genes/'+ expression.split('.txt')[0].split('/')[-1] + '_naive_median_max_test.txt','w')
	out.write('Pathway\tMedian_F1_average\tMedian_F1_std\tMax_F1_average\tMax_F1_std\n')
	for pathway in Result:
		out.write('%s\t%s\t%s\t%s\t%s\n'%(pathway,np.mean(Result[pathway]['Median']),np.std(Result[pathway]['Median']),np.mean(Result[pathway]['Max']),np.std(Result[pathway]['Max'])))
	...
	gene_prediction = pd.DataFrame.from_dict(Gene_prediction, orient='index')  ### make a dataframe from a dictionary
	gene_prediction.to_csv('./Result_on_test_genes/'+ expression.split('.txt')[0] + '_gene_predicted_test.txt', index=True, header=True,sep="\t")
