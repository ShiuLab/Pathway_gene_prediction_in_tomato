...
for files in os.listdir('./'):
	if files.endswith('_complete_gene_predicted.txt'):
		name = files.split('_complete_gene_predicted.txt')[0].split('Multiclass_')[1]
		...
		for pred in [df.Max_1,df.Max_2,df.Max_3,df.Max_4,df.Max_5]:
			...
			if m_max == 0:
				...
				max_result = pd.concat([max_result,a],axis=1)
			m_max += 1
		...
		for pred in [df.Median_1,df.Median_2,df.Median_3,df.Median_4,df.Median_5]:
			...
			if m_median == 0:
				...
				median_result = pd.concat([median_result,a],axis=1)
			m_median += 1
...
F1_median.to_csv("/mnt/home/peipeiw/Documents/Pathway_prediction/20180827_all_EC_pathway/AucROC_PCC/Naive_median_F1_validation.txt",index=True, header=True,sep="\t")
F1_max.to_csv("/mnt/home/peipeiw/Documents/Pathway_prediction/20180827_all_EC_pathway/AucROC_PCC/Naive_max_F1_validation.txt",index=True, header=True,sep="\t")
