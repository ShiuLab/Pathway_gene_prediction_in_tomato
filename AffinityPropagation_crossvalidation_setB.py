...
def main():
	...
	for damping in [0.5,0.6,0.7,0.8,0.9,0.99]:
		for cv_number in range(1,6):
			...
			clustering.fit(mat)
			train_labels = clustering.labels_   # Get cluster assignment labels
			...
			cv_labels = clustering.predict(mat_validation)
			...
			test_labels = clustering.predict(mat_test)
		...
		out = open(save_path+DF + "_AffinityPropagation_%s_%s_results.txt"%(damping,n_clusters), 'w')
		...
		out.close()
			# results = results.rename(columns={0 : 'Gene', 1 : '%s_AffinityPropagations_%s'%(name,n_clusters)})	results.to_csv('/mnt/home/peipeiw/Documents/Pathway_prediction/20180827_all_EC_pathway/Cross_validation/AffinityPropagations_' + name + '_%s_%s_training'%(n_clusters,cv_number), index=False, header=True,sep="\t")
			# cv_results = pd.DataFrame([X_validation.index,cv_labels]).T
			# cv_results = cv_results.rename(columns={0 : 'Gene', 1 : '%s_AffinityPropagations_%s'%(name,n_clusters)})	cv_results.to_csv('/mnt/home/peipeiw/Documents/Pathway_prediction/20180827_all_EC_pathway/Cross_validation/AffinityPropagations_' + name + '_%s_%s_validation'%(n_clusters,cv_number), index=False, header=True,sep="\t")
