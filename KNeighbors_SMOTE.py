...
def DefineClf_KNeighbors(n_neighbors,weights,metric):
	from sklearn.neighbors import KNeighborsClassifier
...
def main():
	...
	CV_performance = {}	# CV_performance [weights,metric,n_neighbors]=[F1_macro,F1_macro,...]
	parameters = {'weights':['uniform', 'distance'], 'metric': ['euclidean','manhattan','minkowski'], 'n_neighbors': [3,4,5,10,15,20,25]}
	for weights in parameters['weights']:
		for metric in parameters['metric']:
			for n_neighbors in parameters['n_neighbors']:
				clf = DefineClf_KNeighbors(n_neighbors,weights,metric)
				for cv_number in range(1,6):
					...
					if 'macro_f1' in result:
						if '%s_%s_%s'%(n_neighbors, weights, metric) not in CV_performance:
							CV_performance['%s_%s_%s'%(n_neighbors, weights, metric)] = []				
						CV_performance['%s_%s_%s'%(n_neighbors, weights, metric)].append(result['macro_f1']) 
	...
	n_neighbors,weights,metric = parameter.split('_')
	...
	for cv_number in range(1,6):
		...
		joblib.dump(clf, save_path + short_name + "_KNeighbors_SMOTE_%s_%s.pkl"%(cv_number,dataset))
...
###### save the Output ######
	if len(classes) > 2:
		...
		# save scores file
		scores_file = save_path + short_name + "_KNeighbors_%s_validation_scores.txt"%dataset
		out_scores = open(scores_file,"w")
		out_scores.write("#ID\t"+pd.DataFrame.to_csv(df_proba,sep="\t").strip()+"\n")
		out_scores.close()
		
		scores_file = save_path + short_name + "_KNeighbors_%s_test_scores.txt"%dataset
		out_scores = open(scores_file,"w")
		...
		out = open(save_path + short_name + "_KNeighbors_%s_results.txt"%dataset, 'w')
		out.write('ID: %s\nAlgorithm: KNeighbors\nTrained on classes: %s\nNumber of features: %i\n' % (DF, classes, n_features))
		out.write('Parameters used:n_neighbors:%s\nweights:%s\nmetric:%s\n' % (n_neighbors,weights,metric))
		...
		Prediction_validation.to_csv(save_path + short_name + "_KNeighbors_%s_validation_prediction.txt"%dataset,index=True, header=True,sep="\t")
		Prediction_testing.to_csv(save_path + short_name + "_KNeighbors_%s_testing_prediction.txt"%dataset,index=True, header=True,sep="\t")
