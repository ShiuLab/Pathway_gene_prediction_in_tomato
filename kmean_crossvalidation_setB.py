...
def Performance_MC(y, pred, classes):
	...
	# if len(y.unique()) == len(pred.unique()) and len(list(set(y.unique())&set(pred.unique()))) == len(y.unique()):
		# macro_f1 = f1_score(y, pred, average='macro')	#
		# f1 = f1_score(y, pred, average=None)	# Returns F1 for each class	. This has some issues when applied to test data
	# else:
...
def main():
	...
	for n_clusters in [5,10,25,50,85,100,200,300,400,500]:
		...
		out = open(save_path+DF + "_Kmean_%s_results.txt"%n_clusters, 'w')
